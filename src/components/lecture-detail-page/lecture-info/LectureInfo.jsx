      ...
      <article className="container">
        <section className="introduction">
          ...
          <AuthorCommentWrapper>
            ...
            {/* {intro.authorComment.map((comment, index) => (
              <React.Fragment key={index}>
                {comment.letter && <p key={index}>{comment.letter}</p>}
                {comment.gif && <img src={comment.gif} alt={comment.label} />}
                {comment.label && <p>{comment.label}</p>}
              </React.Fragment>
            ))} */}
          </AuthorCommentWrapper>
          <Hr />
...
const LectureInfoWrapper = styled.div`
  ...
  .container {
    width: 45rem;
    padding: 1.75rem 1rem 0 2.125rem;
  }
